...
{
    // init the file pointer, lexeme list, tempString, and list counter
    FILE *ifp;
    lexeme list[1000];
    char tempString[100] = {""}, c;
    int listCount = 0;

    // open the file, exit if failure
    ifp = fopen(argv[1], "r");
    if (ifp == NULL) {
    ...
    // print the source code to the terminal
    printFile(ifp);
    ...
    printLexemeTable(list, listCount);

    // print the lexeme list
    printLexemeList(list, listCount);
